...
export class DailyRecord {
	...
	async fetch() {
		try {
			...
			logMessage(
				`${
					I18N_MAP[this.locale][
						`${ERROR_MESSAGE}DAILY_RECORD_FETCH_FAILED`
					]
				}: ${error}`,
				LogLevel.error
			);
	...
	sync = async () => {
		if (!this.settings.dailyRecordAPI) {
			logMessage(
				I18N_MAP[this.locale][`${ERROR_MESSAGE}NO_DAILY_RECORD_API`]
			);
			return;
		}
	...
	async downloadResource() {
		...
		try {
			...
			if (Array.isArray(data)) {
				await Promise.all(
					data.map(async (resource) => {
						...
						// TODO: 修改为obsidian的默认附件路径
						const folder = `${this.settings.dailyNotePath}/Attachments`;
			...
			logMessage(
				`${
					I18N_MAP[this.locale][
						`${ERROR_MESSAGE}RESOURCE_FETCH_FAILED`
					]
				}: ${error}`,
				LogLevel.error
			);
	...
	insertDailyRecord = async () => {
		...
		if (
			!records.length ||
			mostRecentTimeStamp * 1000 < Number(this.lastTime)
		) {
        ...
        console.log(link)
				...
				if (targetFile instanceof TFile) {
					...
					for (const record of localRecordList) {
						...
						const createdTs = regMatch?.length
							? regMatch[1]?.trim()
							: "";
						...
						if (createdTs) {
							...
							if (regMatch) {
								...
								if (localRecordListWithTime[timeStamp]) {
									// 避免时间戳重复，导致相互覆盖
									localRecordListWithTime[timeStamp + 1] =
										record;
								} else {
